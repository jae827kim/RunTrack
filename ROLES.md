...
| 주차 | 작업 |
|------|------|
| 1-2주 | 요구사항 정리, ERD 설계, 테이블 구조 확정 |
| 3-4주 | ORM 모델, Pydantic 스키마, 공통 규칙 정리 |
| 5-6주 | 통합 테스트 시나리오 작성, 팀 API 명세 싱크 |
| 7-8주 | 통합 테스트 실행, 이슈 정리 및 범위 조정 |
| 9-10주 | 전체 시스템 점검, 발표 자료 초안 작성 |
| 11-12주 | 최종 점검, 발표 자료 마무리, 제출 준비 |

---

## 🔵 Backend 1 - 조병현 (사용자 & 신발 관리)
...
| 주차 | 작업 |
|------|------|
| 1-2주 | 사용자/신발 API 명세 정리, 인증 흐름 설계 |
| 3-4주 | 회원가입/로그인, 프로필 API 구현 |
| 5-6주 | 신발 CRUD, 통계 API, 예외 처리 구현 |
| 7-8주 | 테스트 작성, 프론트 연동 이슈 수정 |
| 9-10주 | 보안 점검, 버그 수정, 응답 형식 정리 |
| 11-12주 | 최종 검수 지원, 시연용 데이터 정리 |

---

## 🟣 Backend 2 - 김경빈 (러닝 기록 & 기상 정보 & AI)
...
POST   /api/running/{id}/end             - 러닝 세션 종료
...
POST   /api/recommendations/shoes        - AI 신발 추천 (라우트 추가 예정)
```
...
| 주차 | 작업 |
|------|------|
| 1-2주 | 러닝/기상/AI API 명세 정리, 외부 API 조사 |
| 3-4주 | 러닝 기록 API, 기본 통계 로직 구현 |
| 5-6주 | 기상 조언 API, AI 추천 초안 구현 |
| 7-8주 | 프론트 연동, 통계 보강, 프롬프트 개선 |
| 9-10주 | 버그 수정, 성능 개선, 추천 점수 구현 여부 확정 |
| 11-12주 | 최종 테스트, 시연 흐름 점검, 발표 지원 |

---

## 🟢 Frontend - 이아림 (Kotlin/Android)
...
| 주차 | 작업 |
|------|------|
| 1-2주 | 화면 흐름 정의, 와이어프레임, 디자인 방향 정리 |
| 3-4주 | 로그인/홈 화면, 기본 네비게이션 구현 |
| 5-6주 | 러닝 기록/신발 관리 화면 구현 |
| 7-8주 | AI 추천 화면, API 연동, 상태 처리 |
| 9-10주 | UI 버그 수정, 예외 처리, 선택 기능 UI 판단 |
| 11-12주 | 시연 시나리오 정리, 최종 다듬기 |

---

## 🟠 DevOps & QA - 김주현 (배포 & 테스트)
...
| 주차 | 작업 |
|------|------|
| 1-2주 | 개발 환경 통일, Docker 기본 구조 설계 |
| 3-4주 | GitHub Actions CI, 테스트 자동화 기초 구축 |
| 5-6주 | 배포 파이프라인 초안, 환경변수/시크릿 정리 |
| 7-8주 | 스테이징 배포, 로깅/모니터링 설정 |
| 9-10주 | 회귀 테스트, 성능 점검, 배포 리허설 |
| 11-12주 | 최종 배포 검증, 발표 당일 지원 |

---

## 📅 전체 개발 일정 (12주)

| 주차 | PM | Backend 1 (조병현) | Backend 2 (김경빈) | Frontend (이아림) | DevOps & QA (김주현) |
|------|-----|------|------|------|------|
| **1-2주** | 요구사항 확정, ERD/모델 설계 | 인증 흐름, API 명세 | 러닝/기상 API 명세 | 화면 흐름, 디자인 초안 | 개발 환경, CI 초안 |
| **3-4주** | 공통 스키마, 테스트 기준 정리 | 회원/프로필 API 구현 | 러닝 기록 API 구현 | 로그인/홈, 네비게이션 | Docker, 테스트 자동화 |
| **5-6주** | 통합 시나리오 작성 | 신발 API, 예외 처리 | 기상 조언, AI 추천 초안 | 기록/신발 화면 구현 | 배포 파이프라인 구성 |
| **7-8주** | 통합 테스트 실행 | 프론트 연동, 버그 수정 | 통계 로직, 프론트 연동 | 추천 화면, API 연동 | 스테이징, 로그/모니터링 |
| **9-10주** | 범위 조정, 발표 초안 | 보안/응답 형식 보강 | 성능 개선, 선택 기능 판단 | UI 다듬기, 버그 수정 | 회귀 테스트, 성능 점검 |
| **11-12주** | 최종 점검, 발표 자료, 제출 | 최종 검수 지원 | 최종 검수 지원 | 시연 흐름 정리 | 최종 배포/검증 |
